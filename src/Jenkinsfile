  ...
  stages {
   stage('Git CheckOut') {
      steps {
        ...
        git 'https://github.com/KunalSawant29/Banking-finance.git'
            }
    }
    ...
    stage('Docker Image Creation') {
      steps {
        sh 'docker build -t kunalsawant29/bankingfinance:1.0 .'
            }
    }
    stage('DockerLogin') {
      steps {
    ...
    stage('Push Image to DockerHub') {
      steps {
        sh 'docker push kunalsawant29/bankingfinance:1.0'
            }
    } 
